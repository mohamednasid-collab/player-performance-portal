...
import { categoryAverage, emptyAreaRatings, validateAreaRatings } from '../lib/ratings'
import { AssessmentAreaForm } from './AssessmentAreas'
...
export function EntityForm({ request, teams, players, sessions, onClose, onSaved }: { request: FormRequest; teams: Team[]; players: Player[]; sessions: TrainingSession[]; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  const [areaRatings,setAreaRatings] = useState(emptyAreaRatings)
  ...
  async function submit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (kind === 'assessment') {
        ...
        validateAreaRatings(areaRatings)
        await createAssessment({ team_id: teamId, player_id: playerId, session_id: sessionId || null, assessment_date: text('date'), area_ratings: areaRatings, technical: categoryAverage(areaRatings,'technical'), tactical: categoryAverage(areaRatings,'tactical'), physical: categoryAverage(areaRatings,'physical'), mental: categoryAverage(areaRatings,'mental'), attitude: categoryAverage(areaRatings,'attitude'), comments: optional('comments') })
      }
  ...
  return <Modal title={title} onClose={onClose} busy={busy}><form onSubmit={submit}><fieldset disabled={busy} className="form-grid">
    ...
    {kind === 'assessment' && <><label>Player<select required value={playerId} disabled={!!request.player} onChange={e=>setPlayerId(e.target.value)}><option value="">Select player</option>{players.filter(p=>p.team_id===teamId).map(p=><option key={p.id} value={p.id}>{p.full_name}</option>)}</select></label><label>Session<select value={sessionId} onChange={e=>setSessionId(e.target.value)}><option value="">No linked session</option>{sessions.filter(s=>s.team_id===teamId).map(s=><option key={s.id} value={s.id}>{s.session_date} — {s.title}</option>)}</select></label><AssessmentAreaForm ratings={areaRatings} onChange={setAreaRatings}/><label className="span-2">Comments<textarea name="comments"/></label></>}
