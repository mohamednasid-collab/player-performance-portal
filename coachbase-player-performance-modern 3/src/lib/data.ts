...
import { metrics, validAverage, validateAreaRatings, categoryAverage } from './ratings'
...
export async function getPortalData(profile: Profile): Promise<PortalData> {
  ...
  const assessments = a
  if (assessments.some(a => metrics.some(k => a[k] !== null && !validAverage(a[k])))) throw new Error('Legacy ratings detected. Run the supplied Supabase migration before using this version.')
...
export async function createAssessment(input: Omit<Assessment, 'id' | 'assessed_by'>) {
  validateAreaRatings(input.area_ratings)
  for (const k of metrics) input[k] = categoryAverage(input.area_ratings,k)
