import rubric from '../../src/data/assessment-rubric.json' with { type: 'json' }
...
test('detailed assessment requires 47 scores and previews category averages',async({page})=>{
 await setup(page);await page.goto('/players/p1');await page.getByRole('button',{name:'Review / Assess Player'}).click()
 const dialog=page.getByRole('dialog');await expect(dialog.getByText('0 / 47 areas rated')).toBeVisible();await dialog.getByRole('button',{name:'Save',exact:true}).click();await expect(dialog).toBeVisible()
 for(const area of rubric.technical)await dialog.getByRole('combobox',{name:area.label,exact:true}).selectOption(area.id==='ball_control'?'5':'4')
 await expect(dialog.getByTestId('average-technical')).toContainText('4.1 / 5');await expect(dialog.getByText('10 / 47 areas rated')).toBeVisible()
 await expect(dialog.getByText('First touch, receiving and keeping the ball under pressure')).toBeVisible()
 await page.setViewportSize({width:390,height:844});expect(await dialog.evaluate(el=>el.scrollWidth<=el.clientWidth)).toBe(true)
 await page.screenshot({path:'test-results/detailed-assessment-mobile.png',fullPage:true})
})
