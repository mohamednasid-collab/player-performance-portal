...
async function setup(page:Page,role='administrator'){
 ...
 const players=[{id:'p1',team_id:'a',full_name:'Ali Ahmed',active:true,position:'Forward',jersey_number:9,date_of_birth:'2012-02-10',preferred_foot:'Right',notes:'Strong finisher',linked_user_id:null},{id:'p2',team_id:'b',full_name:'Other Player',active:true,position:'Defender'}]
...
test('manager sees own team only and direct links fail closed',async({page})=>{
 ...
 await page.goto('/players/p1');await expect(page.getByRole('button',{name:'Review / Assess Player'})).toHaveCount(0);await page.goto('/sessions/s1');await expect(page.getByRole('button',{name:'Edit Session'})).toHaveCount(0);await expect(page.getByRole('button',{name:'Delete Session'})).toHaveCount(0)
})
